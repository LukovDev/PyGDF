...
- Модуль ```lib``` - Это папка, в которой хранятся скрипты, которые представляют из себя весь функционал этого фреймворка.
  ...
  Помните, что мы игнорируем только 2 папки в этом фреймворке. Это ```build/tools``` и ```src/lib```. Никак не взаимодействуйте с ними. Не перемещайте, и не переименовывайте.</br>
