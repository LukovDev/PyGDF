...
### Важно! После того как вы установили ```Python 3.11.x``` и убедились что он работает в консоле, запустите файл ```pypi.bat```</br>в папке проекта (фреймворка) чтобы установить все необходимые для фреймворка библиотеки.
...
- Модуль ```lgfw``` - Это папка, в которой хранятся скрипты, которые представляют из себя весь функционал этого фреймворка.</br>
  Это по факту "библиотека" которую можно назвать "ядром" или "движком" этого фреймворка. В эту папку мы не лезем, потому что иначе, вы можете сломать ядро фреймворка.</br>
  Помните, что мы игнорируем только 2 папки в этом фреймворке. Это ```build/tools``` и ```src/lgfw```. Никак не взаимодействуйте с ними. Не перемещайте, и не переименовывайте.</br>
...
![](https://github.com/LukovDev/LibGFW/assets/103067811/328a5382-1938-4fe2-a4ae-04091b47f8ae)</br>


#

### Вот и всё. Вы только что прошли краткое обучение о структуре и использовании этого фреймворка.</br>Если вы хотите ознакомиться с API ядра фреймворка, то я написал об этом [тут.](https://github.com/LukovDev/LibGFW/blob/master/README_API.md)
