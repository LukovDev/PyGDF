...
Много чего из этого списка было уже реализовано, и убрано. Осталось только то, что вы видите сейчас.</br>
Например, ускорение пакетной отрисовки, реализации простой системы частиц, поддержки аудио, и так далее.

- [ ] **Сделать 2D освещение:**
  - [ ] Сделать точечный источник света.
  - [ ] Сделать направленный источник света.
  - [ ] Сделать поддержку теней (путём рейкаста).
...
- [ ] **Сделать параметр пикселизации в 2D камеры (через шейдер и кадровый буфер).**

- [ ] **Сделать Glow эффект из текстуры. Другими словами, свечение текстуры, с учётом цветов на её краях.**
...
- [ ] **Сделать базовую поддержку 3D:**
  ...
  - [ ] Добавить поддержку 3D физики в ```gdf.physics```.</br>
