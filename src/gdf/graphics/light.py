...
from .render import RenderCanvas
...
class Light2D:
    ...
    class LightLayer:
        def __init__(self,
            ...
            self.batch     = SpriteBatch()  # Пакетная отрисовка спрайтов.
            self.ambient_framebuffer = RenderCanvas(camera)  # Кадровый буфер окружающего освещения.
            self.light_framebuffer   = RenderCanvas(camera)  # Кадровый буфер источников света.
    ...
    class PointLight:
        ...
        def _render_(self) -> None:
            ...
            # Рисуем шейдер:
            Sprite2D().render(-1, -1, +1, +1, custom_shader=True)
            self.shader.end()
